        ...
        return (
          <View style={styles.container}>
            <View style={styles.navBar}>
              ...
              <Text style={{fontSize: 16, fontFamily: 'Montserrat', color: 'white', textAlign: 'center', marginTop: Platform.OS === 'ios' ? 15 : 0}}>Create Appointment</Text>
            </View>
          <ScrollView>
            <View style={{flexDirection: 'column', width: width-40, alignSelf: 'center'}}>
              <View style={{flexDirection: 'row', marginTop: 15}}>
                <TouchableOpacity style={this.state.client_state == 0 ? [styles.client_view, {backgroundColor: '#63b7b7'}] : styles.client_view} onPress={() => this.setState({client_state: 0, duration: 7})}>
                  <Text style={this.state.client_state == 0 ? [styles.client_text, {color: 'white'}] : styles.client_text}>Client</Text>
                </TouchableOpacity>
                <TouchableOpacity style={this.state.client_state == 1 ? [styles.client_view, {backgroundColor: '#63b7b7'}] : styles.client_view} onPress={() => this.setState({client_state: 1, duration: 7})}>
                  <Text style={this.state.client_state == 1 ? [styles.client_text, {color: 'white'}] : styles.client_text}>Personal</Text>
                </TouchableOpacity>
              </View>

              <Text style={[styles.client_text, {color: 'black', marginTop: 20, textAlign: 'left'}]}>{'Start Time: ' + this.props.data[1] + ', ' + this.props.data[0]}</Text>

              {
                this.state.client_state == 0 ? (
                  <View>
                    <TouchableOpacity style={styles.sub_view} onPress={NavigationActions.selectClient}>
                      <Text style={styles.sub_text}>Client</Text>
                      <Text style={[styles.client_text, {position: 'absolute', right: 40}]}>Lorem lpsum</Text>
                  ...
                  <View style={{width: width, height: 5, marginLeft: -20, marginTop: 15, borderBottomWidth: 0.2}}/>
                )
              }
            ...
            <TouchableOpacity onPress={() => this.pressButton()}>
              <View style={styles.sBtn_view}>
                <Text style={styles.loginBtntext}>{this.state.client_state == 0 ? 'Book Appointment' : 'Book Personal Time'}</Text>
              </View>
            </TouchableOpacity>
            ...
            <Modal isOpen={this.state.open} onClosed={() => this.setState({open: false})} style={styles.container} position={"bottom"} swipeToClose={false}>
              <View style={styles.navBar}>
                ...
                <Text style={{fontSize: 16, fontFamily: 'Montserrat', color: 'white', textAlign: 'center', marginTop: Platform.OS === 'ios' ? 15 : 0}}>Select Service</Text>
              </View>
...
export default connect(mapStateToProps, mapDispatchToProps)(Manualbooking)
